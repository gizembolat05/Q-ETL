...
def generate_tip4_edw_code(script_name, sql_query,type4_query, source_db_name, target_db_name, source_table_name, subject_area, target_table_name,table_description,id_columns, schema="dbo",db_type="4"):
...
source_db_name = f"{source_db_name}"
...
target_df['TransactionDate'] = pd.to_datetime(target_df['TransactionDate'])
target_df['UpdateTime'] = pd.to_datetime(target_df['UpdateTime'])
yesterday = datetime.strptime(yesterday, '%Y-%m-%d').date()

yesterday_df = target_df[(target_df["TransactionDate"].dt.date == yesterday) | (target_df["UpdateTime"].dt.date == yesterday)]
not_yesterday_df = target_df[(target_df["TransactionDate"].dt.date != yesterday) & (target_df["UpdateTime"].dt.date != yesterday)]
dropped_columns = yesterday_df[[*id_columns, "ETLDate"]]

hashed_source_df = source_df.copy()
hashed_source_df = hashed_source_df.sort_index(axis=1)
hashed_target_df = yesterday_df.drop(columns=["ETLDate"])
...
difference_dict = ff.type4_process_dataframes(difference_df, "difference_df", source_suffixed_columns, target_suffixed_columns)
list_of_dicts.extend(difference_dict)

not_updated_df = pd.merge(not_updated_df,dropped_columns, how='inner', on=id_columns)
not_updated_dict = ff.type4_process_dataframes(not_updated_df, "not_updated_df", source_suffixed_columns, target_suffixed_columns)
list_of_dicts.extend(not_updated_dict)

updated_df = pd.merge(updated_df,dropped_columns, how='inner', on=id_columns)
updated_dict = ff.type4_process_dataframes(updated_df, "updated_df", source_suffixed_columns, target_suffixed_columns)
list_of_dicts.extend(updated_dict)

deleted_df = pd.merge(deleted_df,dropped_columns, how='inner', on=id_columns)
deleted_dict = ff.type4_process_dataframes(deleted_df, "deleted_df", source_suffixed_columns, target_suffixed_columns)
list_of_dicts.extend(deleted_dict)

if list_of_dicts:
    list_of_dicts.extend(not_yesterday_df.to_dict('records'))
