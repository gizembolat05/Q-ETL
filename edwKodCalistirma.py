...
def generate_tip3_edw_code(script_name, sql_query, source_db_name, target_db_name, source_table_name, subject_area, target_table_name, date_column, schema="dbo",db_type="3"):
    """ 
    date_column: Partition'lama için kullanılacak tarih sütunu (varsayılan: "IslemTarihi")
    """
    ...
    # Şimdi Tip3 EDW kodunu oluşturalım
    current_date = datetime.now().strftime("%d.%m.%Y")

    #Kullanıcı adı alınıyor
    kullanici_adi = os.getlogin()

    # date_column parametresine göre tarih dönüşümünü yap
    if date_column:
        sql_query = sql_query.replace(f"[{date_column.strip()}]",
                                      f"CONVERT(DATE, CONVERT(DATETIME, [{date_column.strip()}] AT TIME ZONE 'utc' AT TIME ZONE 'Turkey Standard Time'))  as DayId ")

        # WHERE ifadesini ekle
        sql_query += f" WHERE CONVERT(DATE, CONVERT(DATETIME, [{date_column.strip()}] AT TIME ZONE 'utc' AT TIME ZONE 'Turkey Standard Time')) = CONVERT(date,'{{dayId}}')"
...
ff.write_etl_log(subject_area, script_name, etl_log, "Partition numarasını alma", "Başladı")

partition_number = ff.type3_get_partition_number(dayId, target_table_name, target_engine)

ff.write_etl_log(subject_area, script_name, etl_log, "Partition numarasını alma", "Bitti")

ff.write_etl_log(subject_area, script_name, etl_log, "Target tabloda truncate partition", "Başladı")

ff.type3_truncate_partition(partition_number, schema, target_table_name, target_engine)

ff.write_etl_log(subject_area, script_name, etl_log, "Target tabloda truncate partition", "Bitti")

ff.write_etl_log(subject_area, script_name, etl_log, "Exchange partition", "Başladı")

ff.type3_exchange_partition(partition_number, schema, temp_table_name, target_table_name, target_engine)

ff.write_etl_log(subject_area, script_name, etl_log, "Exchange partition", "Bitti")
...
def generate_tip1_edw_code(script_name, sql_query, source_db_name, target_db_name, source_table_name, target_table_name, subject_area, schema="dbo",db_type="1"):
    # Tablo adından script adını oluştur
    script_name = f"{target_table_name}.py"
